...
Mat post_process(Mat input)
{
    PostProcessing p( input );
    Mat fimg = p.FilterImage();
    //p.DisplayImage("a", fimg);
...
Mat extract_signs_content(Mat input, Mat eimg, int t)
{
    Mat signs(input.rows, input.cols, CV_8UC3, Scalar(255, 255, 255));
    input.copyTo(signs, eimg);
    
    int v;
    for (int i = 0; i < signs.rows; i++) {
        for (int j = 0; j < signs.cols; j++) {
            v = (signs.at<Vec3b>(i, j)[0] >= t || signs.at<Vec3b>(i, j)[1] >= t || signs.at<Vec3b>(i, j)[2] >= t)? 255 : 0;
            signs.at<Vec3b>(i, j)[0] = v;
            signs.at<Vec3b>(i, j)[1] = v;
            signs.at<Vec3b>(i, j)[2] = v;
        }
    }
